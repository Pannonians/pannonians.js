...
todoList.forEach(function (less) {
  console.log(
    `${less.text} / ${less.done ? "Done" : "Not done"} / modified at: ${less.time}`);
});
...
for (i = 0; i < 7; i++) {
  ...
  todoList.forEach(function (less) {console.log(less.text + ": " + (less.done ? "Done!" : "NOT done!") +" modified at: " + less.time);
  });

  // 11. Filter and console.log only ones that are done

  console.log(todoList.filter((item) => item.done === true));
}
